...
image_path = r'C:\Users\Francisco\Documents\repos\ftc_python\apphospitais\app.png'
...
if 'amostra_pacientes_hora.xlsx' in data_dict and df_filtered is not None:
    with tab1:
        ...
        # Container 3: Previsão de Séries Temporais
        with st.container():
            st.subheader("Volumetria de Pacientes com Previsão para os Próximos 30 Dias")
            ...
            if 'Data' in df_pacientes_hora.columns and 'Quantidade de Pacientes' in df_pacientes_hora.columns:
                ...
                model = Prophet()
                model.fit(df_volumetria)
        
                # Criar datas futuras para os próximos 30 dias
                ...
                fig_forecast.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat_lower'],
                                                  mode='lines', name='Intervalo Inferior', line=dict(color='gray', dash='dot')))
                fig_forecast.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat_upper'],
                                                  mode='lines', name='Intervalo Superior', line=dict(color='gray', dash='dot')))
...
# ======================================================
# Container 4: Previsão de Séries Temporais por Turno
# ======================================================
        with st.container():
            st.subheader("Previsão de Pacientes por Turno para os Próximos 30 Dias")
            ...
            if 'Turno' in df_pacientes_hora.columns:
                ...
                # Certifique-se de que as colunas corretas são renomeadas
                df_turno.columns = ['ds', 'Turno', 'y']
        
                fig_forecast_turno = go.Figure()
        
                # Treinar o modelo Prophet para cada turno
                for turno in df_turno['Turno'].unique():
                    df_turno_filtrado = df_turno[df_turno['Turno'] == turno]
        
                    # Treinamento para o modelo de previsão por turno
                    model_turno = Prophet()
                    model_turno.fit(df_turno_filtrado[['ds', 'y']])
        
                    # Previsão para os próximos 30 dias
                    future_turno = model_turno.make_future_dataframe(periods=30)
                    forecast_turno = model_turno.predict(future_turno)
        
                    # Gráfico com os dados reais e previsão por turno
                    fig_forecast_turno.add_trace(go.Scatter(x=df_turno_filtrado['ds'], y=df_turno_filtrado['y'],
                                                            mode='lines', name=f'Dados Reais Turno {turno}', line=dict(color='blue')))
                    fig_forecast_turno.add_trace(go.Scatter(x=forecast_turno['ds'], y=forecast_turno['yhat'],
